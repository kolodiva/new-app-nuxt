  ...
  <div>
    ...
    >
      ...
      <v-toolbar-items style="align-items: center">
        <template v-for="(item2, i) in header3.items">
          <span v-if="item2 == '|'" :key="i" :class="header2.css_class"
            >{{ item2 }}
          </span>
          <v-menu v-else-if="item2.submenu" :key="i" offset-y>
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                text
                max-height="40px"
                v-bind="attrs"
                :class="[header3.css_class, { buttonMFMenu: item2.submenu }]"
                v-on="on"
              >
                {{ item2.name }}
              </v-btn>
            ...
            <v-list>
              <v-list-item
                v-for="(subitem2, index) in item2.submenu"
                :key="index"
                @click="111"
              >
                <v-list-item-title :class="subitem2.css_class">{{
                  subitem2.name
                }}</v-list-item-title>
              </v-list-item>
            </v-list>
      ...
      <v-spacer />
      <v-btn text :class="[header3.css_class]"> ГЕО.ИНФ. </v-btn>
    </v-app-bar>
