...
async function sendCreateLead(taskinfo) {
...
export async function getcdata( {id, id2} ) {
  ...
  if (id2 && id2 === process.env.PASS_1C) {
    ...
    if (id) {
      result = await db.queryAppStatSqlExec( "select id_bitrix24 id, guid, name, code, new_article, mesure, weight, weightbrutto, volume, price, planned_price, old_future_price, future_price, minus6_price, url from nomenklators_bitrix24 where code=$1", [id] );
    } else {
      result = await db.queryAppStatSqlExec( "select id_bitrix24 id, guid, name, code, new_article, mesure, weight, weightbrutto, volume, price, planned_price, old_future_price, future_price, minus6_price, url from nomenklators_bitrix24" );
    }
...
const taskinfo = {
  ...
  BITRIXSHORTDATA: {ORDER_ID: orderid, EMAIL: email ? email : '***', PHONE: phone ? phone : '***', FILIAL: filial ? filial : '***', SUM_ORDER: rows[0].sum },
};
